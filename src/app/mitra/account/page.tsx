...
export default function MitraAccountPage() {
	...
	useEffect(() => {
			...
			try {
				...
				if (profile.bankName) {
					setBankForm({
						...
						accountName: profile.bankHolder || "",
					});
				}
			} catch (e: any) {
		...
		api
			.get("/markets")
			.then((data) => setMarkets(data?.items || data || []))
			.catch(console.error);
	}, [user]);
	...
	const handleSaveBank = async () => {
		...
		try {
			await api.put("/mitra/profile", {
				...
				bankAccount: bankForm.accountNumber,
				bankHolder: bankForm.accountName,
			});
			setEditingBank(false);
			toast.success("Rekening payout berhasil disimpan!");
		} catch (e: any) {
						...
						<StatCard
							...
							value={
								storeProfile?.createdAt
									? new Date(storeProfile.createdAt).toLocaleDateString(
											"id-ID",
											{
												day: "numeric",
												month: "short",
						...
						<StatCard
							...
							value={
								dashboardStats?.rating
									? `${dashboardStats.rating} (${dashboardStats.totalReviews})`
									: "-"
							}
							color="rose"
						/>
						...
						<div className="space-y-4">
									...
									>
										<div className="space-y-4 p-5">
											...
											<Field label="Berada di Pasar">
												<SelectInput
													value={
														markets.find((m: any) => m.id === marketId)?.name ||
														""
													}
							...
							>
								...
								<button
									...
									onClick={async () => {
										try {
											await api.put("/mitra/profile", { operatingHours });
							...
							<div className="rounded-sm border border-zinc-200 bg-white p-5 text-center">
								...
								<div className="mt-3 flex items-center justify-center gap-1.5">
									...
									<span className="text-sm font-semibold text-zinc-900">
										{dashboardStats?.rating
											? dashboardStats.rating.toFixed(1)
											: "-"}
									</span>
								...
								{storeProfile?.avatar && (
									<div className="-mx-5 -mb-5 mt-4 border-t border-zinc-200 px-5 py-4">
...
function StatCard({
	...
	return (
		<div className="rounded-sm border border-zinc-200 bg-white p-3 sm:p-4">
			<div
				className={`inline-flex h-8 w-8 items-center justify-center rounded-sm ${colorMap[color]}`}
			>
...
function InfoRow({
	...
	return (
		<div className="flex items-start gap-3">
			<Icon className="mt-0.5 h-4 w-4 shrink-0 text-zinc-400" strokeWidth={2} />
