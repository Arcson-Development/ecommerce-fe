...
export default function MitraProductDetailPage() {
	...
	async function loadProduct() {
		try {
			...
			setHasVariants(
				data.variants?.length > 1 || data.variants?.[0]?.name !== "Standar",
			);
		} catch (e) {
	...
	const openEditMode = (focusPhoto = false) => {
		setForm({
			name: product.name,
		...
		setHasVariants(
			product.variants?.length > 1 || product.variants?.[0]?.name !== "Standar",
		);
		setEditMode(true);
		setTimeout(() => {
	...
	const handleEditSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			toast.error(
				"Gagal update produk: " + (e?.message || "Terjadi kesalahan"),
			);
		} finally {
			setSubmitting(false);
		}
	...
	const handlePhotoUpload = async (file: File) => {
		...
		try {
			const BASE_URL =
				process.env.NEXT_PUBLIC_API_URL || "http://localhost:6670/api";
	...
	const updateVariant = (idx: number, field: string, value: string) => {
		setVariants((v) =>
			v.map((x, i) => (i === idx ? { ...x, [field]: value } : x)),
		);
	};
			...
			<MitraShell>
				<div className="grid grid-cols-1 gap-6 lg:grid-cols-[16rem_1fr]">
					...
					<div className="space-y-4">
						...
						>
							{/* ── 2-COLUMN: INFO + HARGA ── */}
							<div className="grid grid-cols-1 gap-x-10 gap-y-8 p-6 lg:grid-cols-2">
								{/* INFORMASI PRODUK */}
								<fieldset className="space-y-5">
									<legend className="mb-3 w-full border-b border-zinc-200 pb-2 text-xs font-semibold uppercase tracking-wider text-zinc-900">
										Informasi Produk
									</legend>
								...
								<fieldset className="space-y-5">
									...
									{/* Non-variant: Harga + Stok sejajar */}
									...
									{(hasVariants || variants.length > 1) && (
										<div className="space-y-4">
											{variants.map((v, i) => (
												<div
													key={i}
													className="rounded-sm border border-zinc-200 bg-zinc-50 p-4"
												>
													<div className="mb-3 flex items-center justify-between">
														...
														{variants.length > 1 && (
															<button
																...
																className="flex h-7 w-7 items-center justify-center rounded text-zinc-400 transition-colors hover:bg-rose-50 hover:text-rose-600"
															>
											...
											<button
												type="button"
												onClick={addVariant}
												className="flex w-full items-center justify-center gap-2 border border-dashed border-zinc-300 bg-white py-3 text-sm font-medium text-zinc-600 transition-colors hover:border-zinc-500 hover:text-zinc-900"
											>
							...
							<div ref={photoSectionRef} className="border-t border-zinc-200 px-6 py-6">
								...
								<div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
									{photos.map((photo, i) => (
										<div
											key={i}
											className="group relative aspect-square overflow-hidden rounded-sm border border-zinc-200 bg-zinc-50"
										>
											...
											<button
												...
												className="absolute right-1.5 top-1.5 flex h-7 w-7 items-center justify-center rounded-full bg-rose-500 text-white opacity-0 shadow transition-opacity group-hover:opacity-100"
											>
												<X className="h-3.5 w-3.5" strokeWidth={2.5} />
											</button>
										</div>
									))}
									<label className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-sm border border-dashed border-zinc-300 bg-zinc-50 text-xs text-zinc-500 transition-colors hover:border-zinc-500 hover:bg-white hover:text-zinc-900">
							...
							<div className="flex items-center justify-end gap-3 border-t border-zinc-200 bg-zinc-50 px-6 py-4">
								<button
									...
									className="rounded-sm border border-zinc-300 bg-white px-6 py-3 text-sm font-medium text-zinc-700 transition-colors hover:border-zinc-700"
								>
									Batal
								</button>
								<button
									type="submit"
									disabled={submitting}
									className="rounded-sm bg-zinc-900 px-8 py-3 text-sm font-semibold uppercase tracking-wider text-white transition-colors hover:bg-zinc-800 disabled:opacity-50"
								>
						...
						<div className="grid grid-cols-1 gap-6 p-6 md:grid-cols-2">
							...
							>
								...
								<div className="flex flex-wrap gap-2 border-t border-zinc-200 pt-4">
									<button
										type="button"
										onClick={() => openEditMode(false)}
										className="flex items-center gap-1.5 border border-zinc-300 bg-white px-4 py-2.5 text-xs font-medium text-zinc-800 transition-colors hover:border-zinc-700 hover:text-zinc-900"
									>
...
function VariantStockRow({
	...
	const handleSave = async () => {
		setUpdating(true);
		try {
			await api.put(`/products/mitra/variant/${variant.id}/stock`, {
				...
				{editing ? (
					<>
						...
						<button
							onClick={() => {
								setEditing(false);
								setStockInput(String(variant.stock || 0));
							}}
