  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-ink">Pembayaran Global</h1>
          <p className="text-sm text-muted-ink mt-1">Kelola metode pembayaran yang tersedia untuk semua toko</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: "Nonaktif", value: payments.filter((p) => !p.isActive).length, color: "bg-zinc-300 text-ink" },
        ].map((item) => (
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => (
                  <tr key={p.id} className="hover:bg-zinc-50">
                    ...
                    <td className="px-6 py-4 font-mono text-xs text-muted-ink">{p.code}</td>
